...
export type ModelFormat = "gltf" | "3dtiles" | SplatFormat;

export function isSplatFormat(format: string): format is SplatFormat {
    return format === "ply" ||
        format === "spz" ||
        format === "splat" ||
        format === "ksplat" ||
...
export class Viewer {
    ...
    // 当前 Gaussian Splat；Spark 解码后的中心坐标供 Poisson 工具复用
    ...
    async loadModel(url: string, format: ModelFormat): Promise<THREE.Object3D> {
        ...
        if (isSplatFormat(format)) {
            ...
            if (!response.ok) {
                throw new Error(
                    `${format.toUpperCase()} request failed (${response.status} ${response.statusText})`
                );
            }
            return this.loadSplat(await response.arrayBuffer(), `model.${format}`);
        }
    ...
    // 从内存加载 Spark 支持的单文件 Gaussian Splat 格式
    async loadSplat(fileBytes: ArrayBuffer, fileName: string): Promise<SplatMesh> {
        const splat = new SplatMesh({ fileBytes, fileName, extSplats: true });
        try {
    ...
    async generateCollider(
        ...
        if (this.currentSplat) {
            ...
            const pointCloudPly = this.createPointCloudPly(this.currentSplat);
    ...
    // 当前是否加载的是 Gaussian Splat 或点云
    isSplat(): boolean {
        return this.currentSplat !== null;
    }

    // 将 Spark 已解码的 Gaussian 中心编码成 Open3D 可读的二进制 PLY
    private createPointCloudPly(splat: SplatMesh): ArrayBuffer {
        let validCount = 0;
        ...
        if (validCount === 0) throw new Error("No finite point positions found in splat file");

        const header = [
            "ply",
            "format binary_little_endian 1.0",
            "comment generated by collider-forge from decoded Gaussian centers",
        ...
        const result = new ArrayBuffer(headerBytes.length + validCount * 3 * Float32Array.BYTES_PER_ELEMENT);
        new Uint8Array(result, 0, headerBytes.length).set(headerBytes);
        const view = new DataView(result, headerBytes.length);
        let outputIndex = 0;
        splat.forEachSplat((_index, center) => {
            if (!Number.isFinite(center.x) || !Number.isFinite(center.y) || !Number.isFinite(center.z)) {
                return;
            }
            const offset = outputIndex * 3 * Float32Array.BYTES_PER_ELEMENT;
            view.setFloat32(offset, center.x, true);
            view.setFloat32(offset + 4, center.y, true);
            view.setFloat32(offset + 8, center.z, true);
            outputIndex += 1;
        });
        return result;
    }
